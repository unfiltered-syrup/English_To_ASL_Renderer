...
# This video downloader is from the WLASL project on github, I've modified some aspect of it for our use-case, but the
# majority of the code in this file are not original work
...
def download_nonyt_videos(indexfile, saveto='../../raw_videos'):
    ...
    gloss_videos_count = 0
    for entry in content:
        gloss = entry['gloss']
        instances = entry['instances']

        for inst in instances:
            if gloss_videos_count >= 2:
                gloss_videos_count = 0
                break
            video_url = inst['url']
